...
def train_triplet(model, device, optimizer, criterion, epochs, train_loader, valid_loader=None) -> dict:
    ...
    for epoch in range(1, epochs + 1):
        ...
        # in interpreter
        pbar = tqdm(enumerate(train_loader), file=sys.stdout)
        for batch_idx, (anc, pos, neg, target) in pbar:
            anc, pos, neg, target = anc.to(device), pos.to(device), neg.to(device), target.to(device)

            optimizer.zero_grad()
            output, anc_embed, pos_embed, neg_embed = model(anc, pos, neg)
            loss = criterion(output, anc_embed, pos_embed, neg_embed, target)
            acc = calc_acc(output, target)
            loss.backward()
            optimizer.step()
            ...
            acc = train_acc / (batch_idx * train_loader.batch_size + len(anc))
            pbar.set_postfix(epoch=f'{epoch}/{epochs}', loss='{:.6f}, acc={:.3f}'.format(loss, acc))
        pbar.close()
...
def evaluate_triplet(model, device, criterion, data_loader):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (anc, pos, neg, target) in pbar:
            ...
            acc = total_acc / (batch_idx * data_loader.batch_size + len(anc))
            pbar.set_postfix(loss='{:.6f}, acc={:.3f}'.format(loss, acc))
        pbar.close()
